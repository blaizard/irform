...
GitHub.prototype.getProjectBugLink = function () {
	return this.getProjectLink() + "/issues";
}
...
var DocContext = function (config) {
	this.local = (window.location.protocol.indexOf("file") == 0
			|| location.hostname === "localhost"
			|| location.hostname === "127.0.0.1") ? true :  false;
	...
	for (var i in this.download) {
		...
		(function(link, element) {
			docInternalLoad(link, function(data) {
				...
				var u = 0;
				while (Math.abs(bytes) >= 900 && u < units.length - 1) {
					bytes /= 1024;
					++u;
				}
